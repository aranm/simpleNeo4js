...
var simpleNeo4js = (function (){
    ...
    function queryWithCallback(requestParameters){
        ...
        request
            ...
            .send({
                statements:[{
                    statement: requestParameters.cypherQuery,
                    ...
                    case 200:
                        if (result.body.errors.length > 0){
                            if (arraySupport.firstOrNull(result.body.errors, function (error){
                                ...
                                if (requestParameters.retries < requestParameters.retryCount){
                                    requestParameters.retries++;
                            ...
                            requestParameters.callback(allErrors);
                        }
                        else{
                            requestParameters.callback(null, packageData(result.body))
                        }
                        break;
                    default:
                        requestParameters.callback(result.statusCode)
                }
            });
    }
